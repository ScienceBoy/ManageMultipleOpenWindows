...
void AdjustWindowSize(HWND hwnd) {
    ...
    //MessageBoxW(hwnd, std::to_wstring(si.nMax).c_str(), L"Debug Info", MB_OK);
...
void UpdateDynamicMenus() {
    ...
    // Füge die neuen dynamischen Einträge hinzu
    CreateArrangeOnScreenMenu(hMenu);
    CreateMoveToScreenMenu(hMenu);
}
    ...
    switch (uMsg) { // Check the messages
        ...
        case WM_MOUSEMOVE: { // Message when the mouse is moved
            if (isScrolling) { // Check if scrolling
                ...
                if (yPos != si.nPos) { // Check if the position has changed
                    ...
                    ScrollWindowEx(hwnd, 0, deltaY, &scrollRect, &scrollRect, NULL, NULL, 0); 
                    InvalidateRect(hwnd, &scrollRect, TRUE); // Invalidate and redraw the rectangle
                }
                //InvalidateWindow(hwnd); // Invalidate and redraw the rectangle
        ...
        case WM_MOUSEWHEEL: { // Nachricht, wenn das Mausrad gedreht wird
            ...
            if (yPos != si.nPos) { // Überprüfen, ob sich die Position geändert hat
                ...
                ScrollWindowEx(hwnd, 0, delta / WHEEL_DELTA * 30, &scrollRect, &scrollRect, NULL, NULL, 0); // Entfernt SW_INVALIDATE und SW_ERASE
        ...
        case WM_PAINT: {
            ...
            int searchBoxHeight = 10; // Höhe des Suchfelds
            ...
            for (size_t i = 0; i < processNames.size(); ++i) {
                ...
                std::wstring text = expandedState[processName] ? L"\u25BC        " : L"\u25B6        "; // Set the button text
                ...
                text += 
                    ...
                    + L" (" + std::to_wstring(AlreadyOneChecked) + L" / " + std::to_wstring(windows.size()) + L")";
